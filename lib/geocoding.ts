...
async function parseJson(res: Response): Promise<unknown> {
  ...
  try {
    ...
    console.error(`Non-JSON from ${res.url}:`, text.slice(0, 300));
    throw new Error(`Unexpected response from geocoding service (status ${res.status})`);
  }
}
...
export async function geocodeAddress(address: string): Promise<GeocodeResult> {
  ...
  if (!res.ok) throw new Error(`Address lookup failed (${res.status}). Please try again.`);

  const data = await parseJson(res) as Array<Record<string, string>>;
...
export async function getCensusTract(
  ...
  try {
    ...
    console.error('Census tract lookup failed:', e);
    return null;
  }
}
